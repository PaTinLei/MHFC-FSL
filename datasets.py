...
class DataSet(Dataset):
    ...
    def __init__(self, data_root, setname, img_size):
        ...
        csv_path = osp.join(data_root, 'images',setname + '.csv')
...
class CategoriesSampler():
    ...
    def __iter__(self):
        for i_batch in range(self.n_batch):
            ...
            #print("trad_classes",trad_classes)
            for c in trad_classes:
                l = self.m_ind[c]           #该类所有数据索引
                pos = torch.randperm(len(l))[:self.n_per]  #随机打乱
                cls_batch = l[pos]          #取前5个数据
                cls_indicator = np.zeros(self.n_per)
                #print("cls_indicator",cls_indicator)
                cls_indicator[:cls_batch.shape[0]] = 1   
                #print("cls_indicator",cls_indicator)
                if cls_batch.shape[0] != self.n_per:
                    print("!!!!!!!!!!cls_batch.shape[0] != self.n_per!!!!!!!!!!!!!")
